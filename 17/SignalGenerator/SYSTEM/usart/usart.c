...
__weak void USART_RxCompleteCallback(void)
{
	uint16_t i=0;
    // 打印接收到的完整数据帧
    ...
    // 打印接收缓冲区中的数据
    for (i=0; i < (g_usart_rx_sta & 0x3FFF); i++)
    {
        printf("%c", g_usart_rx_buf[i]);
    }
    printf("\n");
...
void USART_UX_IRQHandler(void)
{
    ...
    if (USART_UX->SR & (1 << 5))                /* 接收到数据 */
    {
        ...
        {
            if (g_usart_rx_sta & 0x4000)        /* 接收到v? */
            {
                if (rxdata != 0xfe)             /* 接收到了0x0a? (必须先接收到到0x0d,才检查0x0a) */
                {
